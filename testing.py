...
def load_weights(path: str = MODEL_PATH) -> dict:
    ...
    # Match keys written by CNN_gen1.py: W1, B1, W2, B2, W3, B3
...
def predict_proba(image_path: str, params: dict) -> float:
    ...
    z3 = np.dot(a2, W3) + b3
    a3 = sigmoid(z3)  # probability of apple (label 1)

    return float(a3.squeeze())


def classify(image_path: str, threshold: float = 0.5) -> dict:
    params = load_weights()
    ...
    return {"prediction":"Apple" if prob_apple>threshold else "Orange","Apple" : f"{(prob_apple*100):.4f}%", "Orange" : f"{(prob_orange*100):.4f}%", }
